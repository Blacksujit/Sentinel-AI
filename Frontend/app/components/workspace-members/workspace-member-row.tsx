...
export function WorkspaceMemberRow({
  ...
  const memberRoleLevel =
    ROLE_OPTIONS.find((r) => r.value === member.role.toUpperCase())?.level || 0
  ...
  const canRemoveThisMember =
    canRemove && memberRoleLevel <= maxRoleLevel && !isCurrentUser
      ...
      <div className="flex items-center gap-3">
        <div
          className={cn(
            ...
            isCurrentUser
              ? 'bg-gradient-to-br from-indigo-500 to-purple-500 text-foreground'
      ...
      <div className="flex items-center gap-4">
        ...
        {(canChangeThisMember || canRemoveThisMember) && (
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="w-48 bg-background/95 backdrop-blur-xl border-border"
            >
              {canChangeThisMember && (
                <>
                  ...
                  {ROLE_OPTIONS.filter((r) => r.level <= maxRoleLevel).map(
                    (role) => (
                      <DropdownMenuItem
                        key={role.value}
