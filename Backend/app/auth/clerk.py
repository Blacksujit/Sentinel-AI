...
def decode_clerk_token(token: str) -> Dict[str, Any]:
    ...
    In development, unsigned decode is only allowed when
    CLERK_DEV_UNSAFE_JWT=1 is explicitly set.
    """
    ...
    dev_unsafe = os.getenv("CLERK_DEV_UNSAFE_JWT", "").strip()
    if dev_unsafe != "1":
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="JWT verification key not configured. Set CLERK_JWT_KEY or CLERK_JWT_PUBLIC_KEY. "
                   "For development only, set CLERK_DEV_UNSAFE_JWT=1 to skip verification.",
        )

    # Development only: decode unsigned (only if CLERK_DEV_UNSAFE_JWT=1)
    try:
        decoded = jwt.decode(
            token,
