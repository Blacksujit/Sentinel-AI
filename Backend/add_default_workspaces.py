"""Migration script to add default workspaces to existing organizations."""
...
def migrate():
    ...
    try:
        ...
        for org in orgs:
            ...
            # Get the org owner
            owner_membership = db.query(OrgMembership).filter(
                OrgMembership.org_id == org.id,
                OrgMembership.user_id == org.owner_user_id
            ).first()
            
            if not owner_membership:
                print(f"    ERROR: No owner membership found for org {org.id}")
                continue
